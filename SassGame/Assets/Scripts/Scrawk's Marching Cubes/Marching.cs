...
namespace MarchingCubesProject
{
    public abstract class Marching : IMarching
    {
        ...
        public virtual void Generate(Vector3 origin, IList<float> voxels, int width, int height, int depth, IList<Vector3> verts, IList<int> indices)
        {
            ...
            for (x = 0; x < width - 1; x++)
            {
                for (y = 0; y < height - 1; y++)
                {
                    for (z = 0; z < depth - 1; z++)
                    {
                        ...
                        //Perform algorithm
                        March(origin, x, y, z, Cube, verts, indices);
                    }
                }
            }
        ...
        /// </summary>
        protected abstract void March(Vector3 origin, float x, float y, float z, float[] cube, IList<Vector3> vertList, IList<int> indexList);
